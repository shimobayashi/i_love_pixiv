...
class FamousIllustIdsFetcher
  ...
  def fetch_following_member_ids
    last_page = @pixiv.agent.get("#{Pixiv::ROOT_URL}/bookmark.php?type=user&rest=hide").search('._pager-complex li:nth-last-child(2) a').inner_text.to_i
    puts "last_page: #{last_page}"

    multi = EM::MultiRequest.new
    (1..last_page).each {|p|
      url = "#{Pixiv::ROOT_URL}/bookmark.php?type=user&rest=hide&p=#{p}"
  ...
  def fetch_jobs(member_ids)
    ...
    EM::Iterator.new(urls, 2).each(proc{|url, iter|
      ...
      illust_ids.each {|illust_id|
        jobs[illust_id] = {name: :famous, score_threshold: 30000}
      }
      yield jobs
    })
  end
